...
from .paperpilot_eval import EvalCase, evaluate_run, write_scorecards
from .paperpilot_qa import PaperPilotKnowledgeBase, write_qa_artifact
...
class PaperPilotTaskService:
    """File-backed service core for PaperPilot task APIs."""
    ...
    def __init__(
        ...
        from .paperpilot_benchmarks import BenchmarkRunManager
        from .paperpilot_observability import build_observability
            ...
            try:
                with trace.span(
                    ...
                    if state.get("run_mode") == "manual":
                        ...
                        return state
                    if state.get("run_mode") == "paperpilot":
                        self._run_paperpilot_pipeline(
                            state, observation_parent=pipeline_span
                        )
                    elif state.get("run_mode") != "fake":
                        raise ValueError(
                            "Supported run modes are 'fake', 'paperpilot', 'manual', and 'fail'."
                        )
                    else:
    ...
    def get_article(self, task_id: str):
        from .paperpilot_references import (
    ...
    def get_trace(self, task_id: str):
        state = self._read_state(task_id)
        trace_path = Path(state["output_dir"]) / "paperpilot_trace.jsonl"
    ...
    def get_dashboard_bundle(self, task_id: str):
        ...
        return {
            "project": {
                "name": "PaperPilot Agent",
                "version": "v{0}".format(__version__.rsplit(".", 1)[0]),
                "description": "Service-backed PaperPilot dashboard snapshot",
            },
            "tasks": [state],
    ...
    def get_artifacts(self, task_id: str):
        ...
        pdf_path = output_dir / "paperpilot_report.pdf"
    ...
    def get_artifact_path(self, task_id: str, artifact_name: str):
        allowed = {
            "paperpilot_report.pdf",
            "paperpilot_report.print.html",
        }
    ...
    def query_knowledge_base(
        ...
        **retrieval_options,
        ...
        from .paperpilot_router_llm import build_chat_llm_callable

        state = self._read_state(task_id)
        output_dir = Path(state["output_dir"])
        kb = PaperPilotKnowledgeBase.from_run_dir(output_dir)
        ...
        answer = kb.answer_question(
            ...
            answer_generator=build_chat_llm_callable(
                enabled=state.get("run_mode") == "paperpilot"
            ),
    ...
    def create_enterprise_knowledge_base(
        ...
        from .paperpilot_enterprise_kb import EnterpriseKnowledgeBaseService
    ...
    def create_enterprise_knowledge_base_from_zotero(
        ...
        PAPERPILOT_ZOTERO_ROOT -> repo-local local_zotero_root.txt -> ~/Zotero.
        """
        from .paperpilot_zotero import discover_zotero_papers
    ...
    def _resolve_zotero_root(self, zotero_root: Optional[str] = None) -> str:
        ...
        if os.getenv("PAPERPILOT_ZOTERO_ROOT"):
            candidates.append(Path(os.getenv("PAPERPILOT_ZOTERO_ROOT")))
        ...
        raise ValueError(
            "未找到 Zotero 数据目录：请填写目录，或设置 PAPERPILOT_ZOTERO_ROOT，"
    ...
    def ask_research_agent(
        ...
        **options,
        ...
        from .paperpilot_research_qa import ResearchQAAgent
    ...
    def create_chat_session(
        ...
        **options,
        ...
        from .paperpilot_chat_agent import PaperPilotChatAgent

        return PaperPilotChatAgent(self).create_session(
            title=title,
            topic=topic,
            run_mode=run_mode,
            retriever=retriever,
    ...
    def get_chat_session(self, chat_id: str):
        ...
        return PaperPilotChatAgent(self).get_session(chat_id)

    def send_chat_message(self, chat_id: str, message: str):
        from .paperpilot_chat_agent import PaperPilotChatAgent

        session = PaperPilotChatAgent(self).get_session(chat_id)
        with self.observability.trace(
            "paperpilot.chat",
            ...
            try:
                result = PaperPilotChatAgent(self).send_message(chat_id, message)
    ...
    def list_chat_sessions(self, limit: int = 50):
        ...
        return PaperPilotChatAgent(self).list_sessions(limit=limit)

    def regenerate_chat_message(self, chat_id: str):
        from .paperpilot_chat_agent import PaperPilotChatAgent

        return PaperPilotChatAgent(self).regenerate_last(chat_id)

    def stop_chat_generation(self, chat_id: str):
        from .paperpilot_chat_agent import PaperPilotChatAgent

        return PaperPilotChatAgent(self).stop_generation(chat_id)

    def get_chat_context(self, chat_id: str):
        from .paperpilot_chat_agent import PaperPilotChatAgent

        return PaperPilotChatAgent(self).get_context(chat_id)

    def compact_chat_context(self, chat_id: str, force: bool = True):
        from .paperpilot_chat_agent import PaperPilotChatAgent

        return PaperPilotChatAgent(self).compact_context(chat_id, force=force)

    def restore_chat_context(self, chat_id: str, compaction_id: str):
        from .paperpilot_chat_agent import PaperPilotChatAgent

        return PaperPilotChatAgent(self).restore_context(chat_id, compaction_id)
    ...
    def _conversation_runtime(self):
        ...
        return PaperPilotConversationRuntime(
    ...
    def _run_fake_research(self, state: Dict):
        ...
        (output_dir / "paperpilot_trace.jsonl").write_text(
    ...
    def _run_paperpilot_pipeline(self, state: Dict, observation_parent=None):
        runner = self.pipeline_runner
        if runner is None:
            from .paperpilot_pipeline import run_paperpilot_pipeline_task

            return run_paperpilot_pipeline_task(
    ...
    def _maybe_generate_pdf(self, state: Dict):
        ...
        try:
            if article is None:
                from .paperpilot_pdf import PdfRenderError
            ...
            if renderer is None:
                from .paperpilot_pdf import PaperPilotPdfRenderer

                renderer = PaperPilotPdfRenderer()
            result = renderer.render(
                markdown_path=article,
                output_pdf=pdf_path,
                title=state.get("topic") or "PaperPilot 调研报告",
            )
            artifacts["pdf"] = {
                "status": "ready",
                "name": pdf_path.name,
