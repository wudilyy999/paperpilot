...
def run_production_governance_benchmark(output_dir):
    ...
    from .paperpilot_enterprise_kb import _answer_cache_identity
...
DEFINITIONS = (
    BenchmarkDefinition(
        ...
        "run_paperpilot_public_benchmark.py",
        ("scifact_dir",),
    ...
    BenchmarkDefinition(
        ...
        "Context",
        "adapter_only",
        "适配器已完成，但缺少同模型 full/fixed/PaperPilot 配对预测，不能发布正式分数。",
    ...
    BenchmarkDefinition(
        ...
        "Memory",
        "public_official_llm",
        "全量对比 Recent、FTS Session 与 PaperPilot Memory，统一 Reader 和 LLM Judge。",
...
class BenchmarkRunManager:
    ...
    def _finish_observability(self, manifest):
        from .paperpilot_observability import numeric_scores
...
def _resolve_benchmark_root(explicit=None):
    ...
    if os.getenv("PAPERPILOT_BENCHMARK_ROOT"):
        candidates.append(Path(os.environ["PAPERPILOT_BENCHMARK_ROOT"]))
    candidates.extend(
        [
            Path.home() / "Desktop" / "codex" / "paperpilot-benchmarks",
