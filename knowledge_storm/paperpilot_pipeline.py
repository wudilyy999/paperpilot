...
from .paperpilot_references import materialize_article_references
...
@dataclass
class PaperPilotPipelineConfig:
    topic: str
    topic_for_storm: str
    output_root: str
    output_dir_name: str
...
def build_pipeline_config_from_task_state(state):
    ...
    return PaperPilotPipelineConfig(
        topic=topic,
...
def run_paperpilot_pipeline_task(state, observation_parent=None):
    config = build_pipeline_config_from_task_state(state)
    return run_paperpilot_pipeline(config, observation_parent=observation_parent)


def run_paperpilot_pipeline(config: PaperPilotPipelineConfig, observation_parent=None):
    configure_paperpilot_logging(verbose=config.verbose)
    ...
    try:
        ...
        callback_handler = PaperPilotStageCallback(trace, topic=config.topic)
        trace.end_stage(
            output_summary={
