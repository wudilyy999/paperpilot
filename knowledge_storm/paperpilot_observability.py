    ...
    def __enter__(self):
        ...
        self.remote = self.owner._remote(
            lambda: self.owner.client.start_observation(
                ...
                metadata={
                    **self.metadata,
                    "paperpilot_trace_id": self.trace_id,
        ...
        if remote_parent is not None:
            self.remote = self.owner._remote(
                lambda: remote_parent.start_observation(
                    ...
                    as_type=self.as_type,
                    input=self.input,
                    metadata={**self.metadata, "paperpilot_span_id": self.span_id},
                )
            )
        return self
        ...
        if remote_parent is not None:
            self.remote = self.owner._remote(
                lambda: remote_parent.start_observation(
                    ...
                    metadata={
                        **self.metadata,
                        "paperpilot_generation_id": self.generation_id,
                    },
                )
            )
        return self
...
def build_observability(root_dir, langfuse_client=None):
    offline = any(
        ...
        for name in ("PAPERPILOT_TEST_OFFLINE", "PAPERPILOT_OFFLINE_TESTS")
    )
    enabled = (
        not offline
        and os.getenv("PAPERPILOT_OBSERVABILITY", "").lower() == "langfuse"
    )
    ...
    if enabled and credentials and client is None:
        try:
            ...
            client = None
    return PaperPilotObservability(
        root_dir,
