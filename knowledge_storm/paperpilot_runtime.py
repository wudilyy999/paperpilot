...
class ToolRegistry:
    ...
    def register(self, tool):
        if not getattr(tool, "name", ""):
            raise ValueError("PaperPilot tools must define a non-empty name.")
    ...
    def get(self, name: str):
        if name not in self._tools:
            raise KeyError("Unknown PaperPilot tool: {0}".format(name))
...
class PaperPilotRuntimeSession:
    """Runtime shell for traceable PaperPilot tool calls and hooks."""
    ...
    def __init__(
        ...
        memory: Optional[PaperPilotMemoryStore] = None,
        ...
        self.memory = memory or PaperPilotMemoryStore()
