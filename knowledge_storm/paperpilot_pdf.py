...
def markdown_to_print_html(markdown_text: str, title: str = "PaperPilot 调研报告"):
    try:
        import markdown
    ...
    escaped_title = html.escape(str(title or "PaperPilot 调研报告"))
    return """<!doctype html>
<html lang="zh-CN">
<head>
  <meta charset="utf-8">
  ...
  <style>
    @page {{
      ...
      @bottom-center {{
        content: "PaperPilot · " counter(page) " / " counter(pages);
        color: #64748b;
        font-size: 9pt;
      }}
    }}
...
def discover_browser_executable():
    candidates = [
        os.getenv("PAPERPILOT_PDF_BROWSER", ""),
...
class PaperPilotPdfRenderer:
    ...
    def render(self, markdown_path, output_pdf, title="PaperPilot 调研报告"):
        ...
        with tempfile.TemporaryDirectory(
            prefix="paperpilot-pdf-", ignore_cleanup_errors=True
        ) as profile_dir:
            command = [
                str(browser),
                "--headless=new",
            ...
            try:
                ...
                if (
                    os.getenv("PAPERPILOT_PDF_ALLOW_NO_SANDBOX", "").strip() == "1"
...
def _replace_latex_math(markdown_text, converter):
    ...
    def protect(match):
        ...
        return "PAPERPILOTCODEBLOCK{0}TOKEN".format(len(protected) - 1)
    ...
    for index, value in enumerate(protected):
        converted = converted.replace("PAPERPILOTCODEBLOCK{0}TOKEN".format(index), value)
    return converted
...
def _append_original_references(markdown_text, run_dir):
    ...
    from .paperpilot_references import append_reference_section, load_reference_registry
