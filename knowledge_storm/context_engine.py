...
def estimate_tokens(text: str):
    """Return a deterministic multilingual token estimate for budget governance."""
    units = re.findall(
        r"[A-Za-z0-9_./:-]+|[\u3400-\u9fff]|[^\s]", str(text or "")
    )
    return len(units)


def truncate_to_tokens(text: str, limit: int, counter=estimate_tokens):
    """Truncate text without requiring a model-specific tokenizer."""
...
@dataclass
class _ContextEngineConfigBase:
    ...
    def __post_init__(self):
        # Public benchmark adapters used ``layer_caps``. Keep that constructor
        # contract while using the clearer layer target name internally.
        if self.layer_caps:
...
class _ContextEngineCore:
    def __init__(self, config=None, ledger=None, summarizer=None):
        self.config = config or _ContextEngineConfigBase()
        self.ledger = ledger
...
class ContextEngineConfig(_ContextEngineConfigBase):
    """Accept legacy option names while storing the current context policy."""
    ...
    def __init__(
        ...
        **updates,
        ...
        super().__init__(
            ...
            layer_targets=updates.pop("layer_targets", updates.pop("layer_caps", _ContextEngineConfigBase().layer_targets)),
            absolute_layer_caps=updates.pop("absolute_layer_caps", _ContextEngineConfigBase().absolute_layer_caps),
        )
...
class ContextEventStore:
    """Compatibility facade backed by the current SQLite ledger."""

    def __init__(self, path):
        path = Path(path)
        if path.suffix == ".jsonl":
            path = path.with_suffix(".context.sqlite3")
        self.path = path
...
class ContextEngine:
    """Compatibility facade for the layered context engine."""

    def __init__(self, config=None, store=None, summarizer=None, token_counter=None):
        self.config = config or ContextEngineConfig()
        if isinstance(self.config, _ContextEngineConfigBase) and not isinstance(self.config, ContextEngineConfig):
            core_config = self.config
        else:
            core_config = self.config
        self.store = store
        ledger = getattr(store, "ledger", store if isinstance(store, ContextLedger) else None)
        self.core = _ContextEngineCore(config=core_config, ledger=ledger, summarizer=summarizer)
