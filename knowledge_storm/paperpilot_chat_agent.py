...
from .paperpilot_memory import PaperPilotMemoryStore
from .memory_store import LongTermMemoryService
from .paperpilot_session_recall import SessionRecallStore


class PaperPilotChatAgent:
    """File-backed conversational layer over ResearchQAAgent."""

    def __init__(self, task_service, intent_router: Optional[PaperPilotIntentRouter] = None):
    ...
    def create_session(
        ...
        **options,
        ...
        session = {
            "chat_id": chat_id,
            "title": title or topic or "PaperPilot Chat",
            "topic": topic,
            "run_mode": run_mode,
            "retriever": retriever,
            ...
            "conversation_runtime": "paperpilot-production-runtime",
            "graph_run": {},
            "created_at": _now(),
            "updated_at": _now(),
            "options": options,
            ...
            sessions.append(
                {
                    ...
                    "title": session.get("title") or "PaperPilot Chat",
    ...
    def send_message(self, chat_id: str, message: str) -> Dict:
        ...
        session["conversation_runtime"] = graph_run.get(
                "runtime", "paperpilot-production-runtime"
        )
    ...
    def _context_engine(self, session: Dict):
        from .paperpilot_router_llm import build_context_summarizer_callable
        ...
        return ContextEngine(
            ...
            summarizer=build_context_summarizer_callable(
                enabled=session.get("run_mode") == "paperpilot"
            ),
        )
...
def _casual_chat_answer(
    ...
    if recalled and any(token in text for token in ["记得", "偏好", "之前", "上次"]):
        ...
        answer = (
            "我是 PaperPilot Research Chat Agent 的本地演示层，不是一个单独训练出来的新基础模型。"
            "真实生成能力取决于你配置的 LLM provider；当前 fake 模式使用可复现的本地示例回答，"
            "paperpilot 模式才会调用真实检索和模型。"
        )
        ...
        answer = (
            ...
            "fake 模式不需要 API key，适合本地演示；paperpilot 模式会调用真实检索和 LLM。"
            "如果你更新过代码，请重启 start_paperpilot_service.py，否则浏览器可能还连着旧服务。"
        )
    else:
        answer = (
            "你好，我是 PaperPilot Research Chat Agent。"
