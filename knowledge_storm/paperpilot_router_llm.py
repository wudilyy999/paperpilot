...
Policy:
    - run_mode == "paperpilot" (real research mode) routes through the LLM
    ...
    - run_mode == "fake" (local demo) stays rule-based unless the operator
      explicitly sets PAPERPILOT_ROUTER_LLM=1.
...
def _router_cache_size() -> int:
    try:
        return max(0, int(os.getenv("PAPERPILOT_ROUTER_CACHE_SIZE", "512")))
    except ValueError:
        return 512
...
def _router_output_tokens() -> int:
    ...
    try:
        return min(
            8_192,
            max(768, int(os.getenv("PAPERPILOT_ROUTER_MAX_TOKENS", "2048"))),
        )
    except ValueError:
        return 2_048
...
def _cached_router_completion(
    ...
    Set PAPERPILOT_ROUTER_CACHE_SIZE=0 to disable (fresh decision every turn).
    """
    import litellm
...
def _load_flat_toml_env(path: str = "secrets.toml"):
    ...
    if os.getenv("PAPERPILOT_SECRETS_PATH"):
        candidates.append(Path(os.getenv("PAPERPILOT_SECRETS_PATH")))
...
def build_router_llm_callable(
    ...
    if enabled is None:
        flag = str(os.getenv("PAPERPILOT_ROUTER_LLM", "")).strip().lower()
...
def build_chat_llm_callable(
    ...
    Policy:
        - PAPERPILOT_CHAT_LLM=1 enables the provider, =0 disables it.
        - An unset flag is offline by default. Callers in ``paperpilot`` mode
          must opt in with ``enabled=True``. Merely having a key in the shell
          must never make tests or fake demos call a paid API.
    """
    if enabled is None:
        flag = str(os.getenv("PAPERPILOT_CHAT_LLM", "")).strip().lower()
        enabled = flag in {"1", "true", "yes", "on"}
    if not enabled:
        return None
    config = _resolve_provider_config()
...
def build_judge_llm_callable(
    ...
    explicitly enabled by a real runtime or PAPERPILOT_JUDGE_LLM=1. An unset
    flag falls back to the deterministic local grader.
    """
    if enabled is None:
        flag = str(os.getenv("PAPERPILOT_JUDGE_LLM", "")).strip().lower()
        enabled = flag in {"1", "true", "yes", "on"}
    if not enabled:
        return None
    config = _resolve_provider_config()
...
def build_memory_extractor_callable(enabled: Optional[bool] = None):
    ...
    if enabled is None:
        enabled = str(os.getenv("PAPERPILOT_MEMORY_LLM", "")).lower() in {
            "1", "true", "yes", "on"
        }
    if not enabled:
        return None
...
def build_context_summarizer_callable(enabled: Optional[bool] = None):
    ...
    if enabled is None:
        enabled = str(os.getenv("PAPERPILOT_SUMMARY_LLM", "")).lower() in {
            "1", "true", "yes", "on"
        }
    if not enabled:
        return None
...
def _resolve_provider_config():
    ...
    api_key = (
        os.getenv("PAPERPILOT_ROUTER_API_KEY")
        or os.getenv("{0}_API_KEY".format(provider.upper()))
        or os.getenv("DEEPSEEK_API_KEY")
    )
    api_base = (
        os.getenv("PAPERPILOT_ROUTER_API_BASE")
...
def _router_function_tool():
    return {
        ...
        "function": {
            ...
            "description": "Submit one structured PaperPilot turn plan.",
            "strict": True,
            "parameters": {
                "type": "object",
                "properties": {
...
def build_intent_router(
    ...
    if llm_router is not None:
        return PaperPilotIntentRouter(llm_router=llm_router)
    real_mode = str(run_mode or "").strip().lower() == "paperpilot"
    return PaperPilotIntentRouter(
