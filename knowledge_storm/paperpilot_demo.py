...
from .paperpilot_agents import PaperPilotResearchOrchestrator
from .paperpilot_runtime import PaperPilotRuntimeSession
from .paperpilot_service import PaperPilotTaskService


class DemoSearchTool:
    name = "demo_search"
    description = "Deterministic demo search tool for PaperPilot dashboard data."
    input_schema = {
        "type": "object",
...
def build_demo_bundle(output_dir):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        service = PaperPilotTaskService(root_dir=temp_root / "service", max_concurrent_tasks=2)
        ...
        session = PaperPilotRuntimeSession(
            run_id="demo-agents",
            task_id="demo-agents",
            trace_path=agent_output_dir / "paperpilot_trace.jsonl",
        )
        session.register_tool(DemoSearchTool())
        multi_agent = PaperPilotResearchOrchestrator(
            session=session,
        ...
        task_state["output_dir"] = f"demo://paperpilot_dashboard/tasks/{task['task_id']}"
        article["path"] = "demo://paperpilot_dashboard/storm_gen_article_polished.txt"
        ...
        data = {
            ...
            "service_snapshot": {
                ...
                "output_dir": f"demo://paperpilot_dashboard/tasks/{task['task_id']}",
                "status": "succeeded",
                "run_mode": "fake",
                "retriever": "arxiv",
            },
...
def _sanitize_demo_paths(value, temp_root):
    ...
    if isinstance(value, str):
        return value.replace(str(temp_root), "demo://paperpilot_dashboard")
    return value
