"""Run cumulative PaperPilot retrieval milestones without implicit downloads."""
...
from knowledge_storm.paperpilot_benchmarks import run_production_governance_benchmark
...
BASELINE_REFERENCE = "docs/benchmarks/paperpilot_public_v55_summary.json"
...
def build_parser():
    parser = argparse.ArgumentParser(description="Run a PaperPilot cumulative milestone")
...
def _run_evidence_governance(output_root):
    ...
    run_dir.mkdir(parents=True, exist_ok=True)
    metrics = {
        "benchmark": "paperpilot-evidence-governance-fixed-v1",
...
def _embedding_provider(args):
    ...
    os.environ["PAPERPILOT_OFFLINE_TESTS"] = "1"
