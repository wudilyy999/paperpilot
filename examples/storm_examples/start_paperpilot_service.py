"""Start the PaperPilot FastAPI service for local demos."""
...
def build_parser():
    parser = argparse.ArgumentParser(description="Start PaperPilot Agent service.")
    parser.add_argument("--service-root", default="./results/paperpilot_demo_service")
...
def print_demo_runbook(args):
    ...
    print("PaperPilot service URL: {0}".format(service_url))
    print("Dashboard file: frontend/paperpilot_dashboard/index.html")
    print("Demo lifecycle: submit -> queued -> running -> succeeded/failed")
    print("Use run_mode=fake for a no-key local demo; use paperpilot for real LLM runs.")
