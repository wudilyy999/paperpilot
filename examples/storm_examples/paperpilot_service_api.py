"""
Optional FastAPI adapter for the PaperPilot service core.

Example:
    uvicorn examples.storm_examples.paperpilot_service_api:app --reload
"""
...
DEFAULT_SERVICE_ROOT = Path("./results/paperpilot_service")
DEFAULT_DASHBOARD_DIR = Path(__file__).resolve().parents[2] / "frontend" / "paperpilot_dashboard"
...
def create_app(service_root=DEFAULT_SERVICE_ROOT, dashboard_dir=DEFAULT_DASHBOARD_DIR):
    ...
    service = PaperPilotTaskService(root_dir=service_root)
    ...
    app = FastAPI(title="PaperPilot Agent Service", version=__version__, lifespan=lifespan)
    app.add_middleware(
        CORSMiddleware,
        allow_origins=["*"],
        allow_methods=["*"],
    ...
    def get_dashboard_home():
        ...
        return {"service": "PaperPilot Agent Service", "dashboard": "not found"}
    ...
    def stream_events(task_id: Optional[str] = None, once: bool = False):
        def event_stream():
            yield _sse_event(
                ...
                {
                    "status": "connected",
                    "message": "PaperPilot SSE connected",
                    "task_id": task_id or "",
                },
            )
            last_status = None
