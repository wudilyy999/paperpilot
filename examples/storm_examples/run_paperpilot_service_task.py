"""
Run one PaperPilot task through the service core.

Example:
    python examples/storm_examples/run_paperpilot_service_task.py \
        --topic "retrieval augmented generation evaluation" \
        --run-mode paperpilot \
        --retriever arxiv \
        --llm-provider deepseek \
        --llm-model flash \
        --output-dir ./results/paperpilot_service_real \
        --max-conv-turn 1 \
        --max-perspective 1 \
        --search-top-k 2
"""
...
def build_parser():
    parser = argparse.ArgumentParser(description="Run a single PaperPilot service task.")
    parser.add_argument("--topic", required=True)
    parser.add_argument("--output-dir", default="./results/paperpilot_service")
    parser.add_argument("--output-language", choices=["original", "zh"], default="zh")
    parser.add_argument("--run-mode", choices=["fake", "paperpilot"], default="fake")
...
def main(argv=None):
    ...
    service = PaperPilotTaskService(root_dir=Path(args.output_dir))
    ...
    result = {
        ...
        "trace": str(Path(finished["output_dir"]) / "paperpilot_trace.jsonl"),
    }
