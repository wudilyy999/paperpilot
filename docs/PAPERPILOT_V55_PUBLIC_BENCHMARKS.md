# PaperPilot v5.5：公开论文 RAG Benchmark

## 结论先行

v5.5 将 PaperPilot 的评测从 synthetic seed 和本地 Zotero 候选扩展到两个公开、可复现的数据集：
...
test Answer F1 已与 QASPER v0.3 自带 `qasper_evaluator.py` 精确对拍；Evidence F1 使用官方 `--text_evidence_only` 口径。详细协议与排查过程见 `docs/PAPERPILOT_V55_QASPER_ANSWER_F1.md`。
...
python examples/storm_examples/run_paperpilot_public_benchmark.py `
