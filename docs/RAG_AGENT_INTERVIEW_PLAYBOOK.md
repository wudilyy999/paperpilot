...
**项目实例**：PaperPilot 用 RAG 接入 arXiv、本地 PDF 与 Memory，不把论文事实固化进参数。
...
**项目实例**：PaperPilot 为每层保存中间产物、版本、耗时和错误。
...
**项目实例**：PaperPilot 用 parent-child 邻域补上下文，不依赖无限增大 overlap。
...
**项目实例**：PaperPilot 为 PIM、DPD 等 RF 术语维护 analyzer profile 与 fingerprint。
...
**项目实例**：PaperPilot 验证 Legacy MiniLM、BGE、GTE、Qwen；CPU 默认偏向 GTE，Qwen 质量高但更慢。
...
**项目实例**：PaperPilot manifest 保存维度、归一化、metric 与 revision，拒绝静默复用不兼容索引。
...
**项目实例**：PaperPilot 曾重复 RRF 压平 margin 并导致 100% 重排，后改为仅多查询聚合时再次融合。
...
**项目实例**：PaperPilot 在 Hybrid/RRF 后选择性触发 Cross-Encoder，再进入 Parent Context。
...
**项目实例**：PaperPilot 提供 exact/HNSW 双后端，并用固定小集比较 ANN 与 exact TopK。
...
**项目实例**：PaperPilot 当前本地演示采用 HNSW，百万级扩展再评估分片与 IVF/PQ。
...
**项目实例**：PaperPilot 保存 child-parent 映射、邻域、预算和截断原因。
...
**项目实例**：PaperPilot 先粗过滤再结构化压缩，保存 source span 和原文回退路径。
...
**项目实例**：PaperPilot 摘要包含 facts、open_questions、citations 和 temporal validity。
...
**项目实例**：PaperPilot 组合 Recent、FTS Session 和可选真实 Dense Memory。
...
**项目实例**：PaperPilot 可比较 Recent、FTS Session 和 Dense Memory Profile，Hash 仅供测试。
...
**项目实例**：PaperPilot 的结构化 LLM decision 输出 actions、confidence、reason、error_type，而非硬编码“故事续写”。
...
**项目实例**：PaperPilot 将检索、问答、调研封装为结构化 Tool，并提供 MCP 适配。
...
**项目实例**：PaperPilot 保留本地 Trace 与 Langfuse observation，外部平台失败时业务 fail-open。
...
**项目实例**：PaperPilot 用 fingerprint 区分 SciFact、QASPER、LongMemEval-S 与 PIM pilot。
...
**项目实例**：PaperPilot 将 QASPER retrieval 与独立端到端协议分开报告。
...
**项目实例**：PaperPilot 离线测试用 Hash embedding/fake LLM，公开 Benchmark 单独运行。
...
**项目实例**：PaperPilot 记录 Recall、P95、rerank 触发率、引用治理和 improved/regressed cases。
...
**项目实例**：PaperPilot 增加 unique-parent 最低配额、分数加权剩余预算与 child 邻域展开。
...
**项目实例**：PaperPilot 统一公式规范化，HTML 预览用数学渲染，PDF 路径用支持公式的转换器并保留源码。
...
**项目实例**：PaperPilot 改为动作路由，topic 只作可选 evidence hint，普通聊天默认不检索。
...
**项目实例**：PaperPilot 将输出 token 从固定小值改为 profile 动态预算，保存 finish_reason 和已用 token。
**排查/设计步骤**：检查模型响应、SSE 序号、服务日志和浏览器消息长度；对长故事做端到端回归。
**追问**：动态上限比统一 4000 好在哪里？
**考察点**：生成预算和流式链路。
**常见失误**：只把上限改大，不保留上下文与成本空间。

### 43. “你好，我是 PaperPilot”重复出现的根因是什么？
...
**项目实例**：PaperPilot trace 记录 memory write、index generation、retrieval source、filter 和 selected IDs。
...
**项目实例**：PaperPilot 的 Context 摘要增加 schema、source span 与压缩失败回退。
...
**项目实例**：PaperPilot 的 evidence set 保存 stance 与 temporal validity，生成层不得静默投票。
...
**项目实例**：PaperPilot 将 tenant/user/document 权限下推到 retrieval adapter，并用不可见文档做负向测试。
...
**项目实例**：PaperPilot 为 arXiv adapter 增加超时、有限重试、错误分类和部分成功汇总。
...
**项目实例**：PaperPilot 将可选 source 标记 degraded，继续 arXiv/Local PDF 并在报告中记录缺口。
...
**项目实例**：PaperPilot 将第三方 Message 转成内部 DTO，锁定兼容依赖并降低无用告警噪声。
...
**项目实例**：PaperPilot 小样本中 BGE 并非所有数据都优于 Legacy，Qwen 质量较高但 CPU 延迟显著。
...
**项目实例**：PaperPilot 删除静默吞异常，缺 FastAPI/Uvicorn 时启动直接给出依赖错误。
...
**项目实例**：PaperPilot 将 Dashboard 静态资源由同一 FastAPI app 提供，并加 root、CSS、JS smoke test。
...
**项目实例**：PaperPilot 保存 improved/regressed 案例；SciFact P2 改善 9、回归 1，QASPER 改善 151、回归 52。
...
**项目实例**：可复用 PaperPilot 的本地 PDF、Hybrid、Evidence、Trace 与会话层，替换企业身份和存储适配器。
...
**项目实例**：PaperPilot 的 exact/HNSW 抽象可扩展到 Milvus/FAISS/pgvector，但需真实容量验证。
...
**项目实例**：PaperPilot Service 的任务状态、SSE、幂等和可取消执行可作为控制面基础。
...
**项目实例**：PaperPilot 在 STORM Multi-Agent 主链上扩展 arXiv、本地 PDF、Runtime、RAG 与 Benchmark。
...
**项目实例**：PaperPilot 问答支持普通聊天、知识问答与 Research 升级，而非先手工提交任务。
...
**项目实例**：PaperPilot 用 LongMemEval-S 检查跨会话 retrieval，并区分 Memory 与 Evidence。
...
**项目实例**：PaperPilot 提供 128K/256K/512K Profile，而不是把模型 1M 窗口每次塞满。
...
**项目实例**：PaperPilot 的调研任务、问答与 MCP Tool 可由同一 Runtime 观测和恢复。
...
**项目实例**：PaperPilot 已支持本地 PDF、Hash 测试与本地 Trace，真实离线需替换云 LLM 和 arXiv。
...
**项目实例**：PaperPilot 对可选 Wikipedia fail-open，对权限和索引兼容 fail-closed。
...
**项目实例**：PaperPilot 用本地 Trace 保底、Langfuse 做跨运行聚合，并把 badcase 回流离线集。
...
**项目实例**：PaperPilot 可把 arXiv、本地 PDF 与企业文档统一成 Evidence schema。
...
**项目实例**：PaperPilot manifest/fingerprint 可阻止模型或 analyzer 变化后误用旧索引。
...
**项目实例**：PaperPilot UI 展示问答 token/耗时，Trace 记录每节点成本。
...
**项目实例**：PaperPilot 把检索文本视为不可信数据，不允许文档内容改变系统/工具权限。
...
**项目实例**：PaperPilot 调研流程可保存 conversation、outline、article、trace 与失败节点。
...
**项目实例**：PaperPilot 本地 PDF adapter 是起点，可扩展 SharePoint、Confluence、对象存储。
...
**项目实例**：PaperPilot 统一 SciFact、QASPER、LongMemEval-S 与 PIM pilot，但保持任务协议隔离。
...
**项目实例**：PaperPilot 用 SSE 展示路由、检索、节点状态、token 与生成片段。
...
**项目实例**：PaperPilot 从 STORM 示例演进到论文检索问答、Memory/Context、Eval、Langfuse 与服务 UI。
...
**项目实例**：保留 STORM Agent 协作主链，在外层增加 PaperPilot 工程底座。
...
**项目实例**：PaperPilot 用 arXiv 做公开论文首召回，本地 PDF/Zotero 补充用户已有材料。
...
**项目实例**：PaperPilot 对 arXiv 做查询扩展和领域消歧，对本地 PDF/问答用自建 BM25 + Dense + RRF。
...
**项目实例**：PaperPilot manifest 在加载时校验 fingerprint，不匹配则重建或拒绝。
...
**项目实例**：PaperPilot 使用 n=300 的固定 retrieval 协议报告 Recall@10。
...
### 83. 为什么用 QASPER？
**参考回答**：QASPER 是论文全文问答，问题与证据低词面重叠，能测段落检索、Evidence 与答案链路，贴合 PaperPilot。
...
**项目实例**：PaperPilot 在 cleaned 500/500 retrieval-only 上比较 Recent、FTS Session 与 v5.6 Memory。
...
### 85. PIM 本地数据集是怎么来的？
**参考回答**：从本地 Zotero PIM 论文中选 5 篇，解析为 797 chunks，再由 Flash 模型基于原文生成 50 题并保留证据映射，用于私有 pilot。
...
**项目实例**：QASPER 官方 LED 端到端结果与 PaperPilot retrieval Recall@5 不是同一任务；BEIR SciFact 常报 nDCG@10。
...
**项目实例**：PaperPilot 将 citation mapping 与 claim support 分成两个 evaluator。
...
**项目实例**：PaperPilot 修复 100% 重排并限制候选最多 20，提供超时回退。
...
**项目实例**：PaperPilot 记录 parent_id、邻域、allocation 和 truncation reason。
...
**项目实例**：PaperPilot 采用适配器和可选 LangGraph，而非一次替换全部模块。
**排查/设计步骤**：列能力 gap，做最小垂直切片，对比状态恢复、Trace、测试与性能再决定迁移。
**追问**：哪些模块最值得框架化？
**考察点**：避免重复造轮子与迁移判断。
**常见失误**：把“用了框架”当成熟度。

### 94. Langfuse 在 PaperPilot 中怎样定位 Bad Case？
...
### 95. PaperPilot 的 Agent Runtime 成熟度如何评价？
...
### 99. 如何用 STAR 在三分钟内介绍 PaperPilot？
...
- 项目内 `RAG_BADCASE_PROGRESSIVE_RESULTS.md`、`RAG_BAD_CASES_AND_ROADMAP.md`、`PAPERPILOT_RETRIEVAL_STACK_UPGRADE.md`、`PAPERPILOT_DOMAIN_PILOT.md`：项目数字与案例的事实源。
