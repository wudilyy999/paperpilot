# PaperPilot v7.0 PIM 领域评测

## 目标

该评测验证 PaperPilot 在无源互调（PIM）垂直领域中的完整链路：论文解析、结构化切分、
...
`$env:PAPERPILOT_BENCHMARK_ROOT\domain-pim-v7`，不进入 Git。
...
python examples/storm_examples/prepare_pim_domain_pilot.py `
  ...
  --output-dir "$env:PAPERPILOT_BENCHMARK_ROOT\domain-pim-v7"
```
...
python examples/storm_examples/run_pim_domain_pilot.py `
  ...
  --model-cache <paperpilot-benchmarks\models> `
  --top-k 5
```
