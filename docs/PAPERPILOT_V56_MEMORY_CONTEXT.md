# PaperPilot v5.6 Memory 与 Context 工程复盘
...
没有直接引入 Graphiti/Neo4j 或托管 Memory。原因是 PaperPilot 的目标包含本地演示、离线 CI 和实现可解释性；完整图数据库会显著增加部署与故障面。最终方案采用 SQLite WAL，并保留 embedding/extractor provider 接口，未来可替换外部底座。
...
```text
%USERPROFILE%\Desktop\codex\paperpilot-benchmarks\v56\
```
...
`docs/benchmarks/paperpilot_v56_paid_quarter_summary.json`。
