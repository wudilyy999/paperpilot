...
class PaperPilotMemoryQATest(unittest.TestCase):
    ...
    def test_memory_store_persists_three_memory_layers_and_preferences(self):
        from knowledge_storm.paperpilot_memory import PaperPilotMemoryStore

        store = PaperPilotMemoryStore()
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            store.save(path)
            loaded = PaperPilotMemoryStore.load(path)
    ...
    def test_context_compression_keeps_constraints_and_reports_validation(self):
        from knowledge_storm.paperpilot_memory import compress_context
    ...
    def test_kb_qa_tool_exposes_schema_and_runs(self):
        from knowledge_storm.paperpilot_tools import KnowledgeBaseQATool
    ...
    def test_research_qa_tool_exposes_schema_and_runs_fake_agent(self):
        from knowledge_storm.paperpilot_tools import ResearchQATool
    ...
    def test_evaluate_qa_artifact_scores_grounded_answers(self):
        from knowledge_storm.paperpilot_eval import EvalCase, evaluate_qa_artifact
    ...
    def test_runtime_session_runs_tool_with_trace_and_working_memory(self):
        ...
        trace_path = run_dir / "paperpilot_trace.jsonl"
        session = PaperPilotRuntimeSession(run_id="unit-test", trace_path=trace_path)
