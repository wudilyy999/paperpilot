...
FRONTEND = ROOT / "frontend" / "paperpilot_dashboard"


class PaperPilotUIV57Test(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_product_uses_v57_workspace_shell(self):
        self.assertIn("v1.0.0", self.html)
    ...
    def test_readme_presents_new_screenshots_diagram_and_benchmark_icons(self):
        for marker in (
            ...
            "paperpilot-research-flow-v65.gif",
    ...
    def test_real_pipeline_is_the_default_for_research_and_chat(self):
        self.assertIn(
            '<option value="paperpilot" selected>真实检索与 LLM</option>',
            self.html,
        )
        self.assertIn(
            '<option value="paperpilot" selected>真实 API</option>',
            self.html,
        )
