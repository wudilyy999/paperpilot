...
class PaperPilotBenchmarkRegistryTest(unittest.TestCase):
    def setUp(self):
    ...
    def test_catalog_prioritizes_v55_v56_and_resolves_local_inputs(self):
        from knowledge_storm.paperpilot_benchmarks import BenchmarkRegistry

        with mock.patch.dict(
            os.environ,
            {"PAPERPILOT_BENCHMARK_ROOT": str(self.benchmark_root)},
            clear=False,
        ):
        ...
        self.assertEqual(
            catalog["benchmark_root"], str(self.benchmark_root.resolve())
        )
...
class PaperPilotBenchmarkWorkbenchApiTest(unittest.TestCase):
    def test_fastapi_exposes_catalog_and_validates_paid_run(self):
        ...
        with tempfile.TemporaryDirectory() as temp:
            ...
            with mock.patch.dict(
                os.environ,
                {"PAPERPILOT_BENCHMARK_ROOT": str(root / "missing")},
                clear=False,
            ):
    ...
    def test_frontend_is_a_registry_workbench_not_a_version_museum(self):
        ...
        self.assertIn("result.modes?.paperpilot_memory", script)
