...
class PaperPilotReleaseIntegrityV52Test(unittest.TestCase):
    def test_llm_builders_are_offline_by_default_even_when_key_exists(self):
        from knowledge_storm.paperpilot_router_llm import (
        ...
        clean_environment = {
            ...
            if key not in {"PAPERPILOT_CHAT_LLM", "PAPERPILOT_JUDGE_LLM"}
        }
    ...
    def test_package_versions_match_current_release(self):
        ...
        self.assertEqual(setup_version, "1.0.0")
        self.assertEqual(init_version, setup_version)
        self.assertIn("PaperPilot Agent", setup_text)
    ...
    def test_v55_public_benchmark_artifacts_are_documented_and_sanitized(self):
        readme = (ROOT / "README.md").read_text(encoding="utf-8")
        guide = (ROOT / "docs" / "PAPERPILOT_V55_PUBLIC_BENCHMARKS.md").read_text(
            encoding="utf-8"
        )
        summary = (
            ROOT / "docs" / "benchmarks" / "paperpilot_public_v55_summary.json"
    ...
    def test_ci_runs_offline_unit_tests(self):
        ...
        self.assertIn("PAPERPILOT_CHAT_LLM: 0", workflow)
        self.assertIn("PAPERPILOT_JUDGE_LLM: 0", workflow)
        self.assertIn("PAPERPILOT_RETRIEVAL_EMBEDDING: hash", workflow)
