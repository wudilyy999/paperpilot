...
class PaperPilotRouterLLMTest(unittest.TestCase):
    def test_router_tool_schema_constrains_memory_and_response_contract(self):
        from knowledge_storm.paperpilot_router_llm import _router_function_tool
    ...
    def test_router_completion_uses_function_arguments_as_structured_content(self):
        from knowledge_storm.paperpilot_router_llm import _completion_result
    ...
    def test_router_output_budget_is_large_enough_for_complete_json_and_bounded(self):
        from knowledge_storm.paperpilot_router_llm import _router_output_tokens

        with mock.patch.dict("os.environ", {}, clear=False):
            self.assertEqual(2048, _router_output_tokens())
        with mock.patch.dict(
            "os.environ", {"PAPERPILOT_ROUTER_MAX_TOKENS": "256"}
        ):
            self.assertEqual(768, _router_output_tokens())
        with mock.patch.dict(
            "os.environ", {"PAPERPILOT_ROUTER_MAX_TOKENS": "99999"}
        ):
            self.assertEqual(8192, _router_output_tokens())

    def test_rewrite_query_preserves_original_without_calling_search_planner(self):
        from knowledge_storm.paperpilot_intent_router import rewrite_query
    ...
    def test_dynamic_output_budget_scales_with_response_contract(self):
        from knowledge_storm.paperpilot_router_llm import select_output_budget
    ...
    def test_length_finish_reason_continues_once_and_aggregates_telemetry(self):
        from knowledge_storm.paperpilot_router_llm import complete_chat_with_telemetry
    ...
    def test_fake_mode_router_is_rule_based(self):
        from knowledge_storm.paperpilot_router_llm import build_intent_router
    ...
    def test_injected_llm_router_reaches_graph_result(self):
        ...
        runtime = ProductionRuntime(
            ...
            intent_router=PaperPilotIntentRouter(llm_router=fake_llm),
        )
    ...
    def test_paperpilot_mode_builds_llm_router_when_key_present(self):
        from knowledge_storm.paperpilot_router_llm import build_intent_router

        router = build_intent_router(run_mode="paperpilot")
    ...
    def test_chat_llm_builder_respects_enable_flag(self):
        from knowledge_storm.paperpilot_router_llm import build_chat_llm_callable
    ...
    def test_judge_llm_builder_respects_enable_flag(self):
        from knowledge_storm.paperpilot_router_llm import build_judge_llm_callable
