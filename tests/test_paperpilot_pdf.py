...
from knowledge_storm.paperpilot_pdf import (
    PaperPilotPdfRenderer,
    PdfRenderError,
...
class PaperPilotPdfTest(unittest.TestCase):
...
"""
    ...
    def test_math_conversion_failure_keeps_formula_visible(self):
        from knowledge_storm.paperpilot_pdf import _replace_latex_math
    ...
    def test_renderer_uses_browser_without_shell_and_registers_verified_pdf(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            output_path = Path(temp_dir) / "paperpilot_report.pdf"
    ...
    def test_chrome_retries_with_compatibility_sandbox_flag_only_after_failure(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir, mock.patch.dict(
            "os.environ", {"PAPERPILOT_PDF_ALLOW_NO_SANDBOX": "1"}
        ):
            markdown_path = Path(temp_dir) / "article.md"
    ...
    def test_real_browser_pdf_contains_rendered_formula_text(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir, mock.patch.dict(
            ...
            result = PaperPilotPdfRenderer(browser_path=browser).render(
    ...
    def test_renderer_reports_missing_browser_with_typed_error(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            renderer = PaperPilotPdfRenderer(browser_path="")

            with mock.patch(
                "knowledge_storm.paperpilot_pdf.discover_browser_executable",
                return_value=None,
            ):
                with self.assertRaises(PdfRenderError) as caught:
                    renderer.render(
                        markdown_path,
                        Path(temp_dir) / "paperpilot_report.pdf",
                    )
    ...
    def test_pdf_bibliography_preserves_original_title_and_authors(self):
        from knowledge_storm.paperpilot_pdf import _append_original_references
    ...
    def test_renderer_rejects_missing_or_empty_article(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            renderer = PaperPilotPdfRenderer(browser_path="edge.exe")
