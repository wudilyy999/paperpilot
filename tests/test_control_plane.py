...
class PaperPilotProductionV45Test(unittest.TestCase):
    ...
    def test_service_reuses_legacy_state_roots_during_upgrade(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            (root / "production_runtime_v45").mkdir()
            ...
            self.assertEqual(
                ...
                service._production_runtime().root_dir,
            )
            self.assertEqual(
                legacy_control,
                service._production_control().database_path,
            )
            self.assertEqual(root / "memory_service_v56", service._memory_service().root_dir)

    def test_runtime_uses_one_resolved_graph_root_for_write_and_read(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            (root / "langgraph_v44").mkdir()
            service = TaskService()
            service.root_dir = root
            runtime = ProductionRuntime(root, service)

            self.assertEqual(root / "langgraph_v44", runtime._graph_root())
    ...
    def test_enterprise_kb_enforces_acl_and_processes_incremental_update(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            control = service._production_control()
    ...
    def test_production_api_exposes_governed_runtime(self):
        try:
        ...
        self.assertEqual(status.json()["schema_revision"], 1)
