...
class PaperPilotServiceTest(unittest.TestCase):
    def make_service(self, **kwargs):
        temp_dir = tempfile.TemporaryDirectory()
        self.addCleanup(temp_dir.cleanup)
        from knowledge_storm.paperpilot_service import PaperPilotTaskService

        return PaperPilotTaskService(root_dir=Path(temp_dir.name), **kwargs)
    ...
    def test_resolve_zotero_root_prefers_explicit_path_then_env(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with mock.patch.dict(
                os.environ, {"PAPERPILOT_ZOTERO_ROOT": temp_dir}, clear=False
            ):
    ...
    def test_dashboard_bundle_collects_task_artifacts_for_frontend(self):
        ...
        self.assertEqual(bundle["project"]["version"], "v1.0")
    ...
    def test_fastapi_adapter_imports_without_required_runtime_dependency(self):
        from examples.storm_examples import paperpilot_service_api

        self.assertTrue(hasattr(paperpilot_service_api, "create_app"))
        self.assertTrue(hasattr(paperpilot_service_api, "DEFAULT_SERVICE_ROOT"))
    ...
    def test_fastapi_adapter_serves_dashboard_home_and_sse_events(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertIn("PaperPilot Agent Workspace", home.text)
    ...
    def test_fastapi_serves_only_allowlisted_task_artifacts(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            (output_dir / "paperpilot_report.pdf").write_bytes(b"%PDF-test")

            pdf = client.get(
                "/research-tasks/{0}/artifacts/paperpilot_report.pdf".format(
                    task["task_id"]
                )
            )
            ...
            missing = client.get(
                "/research-tasks/{0}/artifacts/paperpilot_report.print.html".format(
                    task["task_id"]
                )
            )
    ...
    def test_fastapi_minimal_research_request_uses_balanced_profile(self):
        ...
        self.assertEqual(task["run_mode"], "paperpilot")
    ...
    def test_paperpilot_run_mode_uses_injected_pipeline_runner(self):
        calls = []
        ...
        self.assertEqual(calls[0]["run_mode"], "paperpilot")
    ...
    def test_paperpilot_run_mode_records_structured_runner_failure(self):
        def runner(state):
    ...
    def test_retry_clears_stale_error_after_success(self):
        ...
        service = self.make_service_with_pipeline_runner(runner)
        task = service.submit_research_task(
            topic="Muon optimizer", run_mode="paperpilot"
        )
    ...
    def test_optional_pdf_is_registered_and_exposed_in_dashboard_bundle(self):
        ...
        artifact_path = service.get_artifact_path(
            task["task_id"], "paperpilot_report.pdf"
        )
        ...
        self.assertTrue(bundle["artifacts"]["pdf"]["url"].endswith("paperpilot_report.pdf"))
    ...
    def test_pdf_failure_keeps_research_success_and_reports_typed_error(self):
        from knowledge_storm.paperpilot_pdf import PdfRenderError
    ...
    def test_artifact_lookup_is_allowlisted_and_rejects_traversal(self):
        ...
        service.run_task(task["task_id"])
        output_dir = Path(service.get_task(task["task_id"])["output_dir"]).resolve()
        (output_dir / "paperpilot_report.pdf").write_bytes(b"%PDF-test")

        self.assertEqual(
            service.get_artifact_path(task["task_id"], "paperpilot_report.pdf"),
            output_dir / "paperpilot_report.pdf",
        )
