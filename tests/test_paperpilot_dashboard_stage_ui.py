...
class PaperPilotDashboardStageUiTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        root = Path(__file__).resolve().parents[1]
        cls.html = (root / "frontend/paperpilot_dashboard/index.html").read_text(
            encoding="utf-8"
        )
        cls.javascript = (
            root / "frontend/paperpilot_dashboard/app.js"
        ).read_text(encoding="utf-8")
        cls.css = (root / "frontend/paperpilot_dashboard/styles.css").read_text(
            encoding="utf-8"
        )
    ...
    def test_deliver_card_has_pdf_choice_and_open_action(self):
        ...
        self.assertIn("paperpilot_report.pdf", self.javascript)
