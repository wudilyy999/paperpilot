...
class PaperPilotPipelineTest(unittest.TestCase):
    ...
    def test_pipeline_defaults_use_balanced_research_profile(self):
        from knowledge_storm.paperpilot_pipeline import PaperPilotPipelineConfig

        config = PaperPilotPipelineConfig(
            topic="Muon optimizer",
            topic_for_storm="Muon optimizer",
            output_root="C:/tmp/paperpilot",
            output_dir_name="muon",
            article_dir="C:/tmp/paperpilot/muon",
        )
    ...
    def test_build_pipeline_config_from_task_state_uses_task_output_dir(self):
        from knowledge_storm.paperpilot_pipeline import build_pipeline_config_from_task_state
        ...
        state = {
            ...
            "retriever": "arxiv",
            "output_language": "zh",
            "output_dir": "C:/tmp/paperpilot/tasks/task-123",
        ...
        self.assertEqual(config.output_root, str(Path("C:/tmp/paperpilot/tasks")))
        self.assertEqual(config.output_dir_name, "task-123")
        self.assertEqual(config.article_dir, str(Path("C:/tmp/paperpilot/tasks/task-123")))
    ...
    def test_build_pipeline_config_supports_local_pdf_options(self):
        ...
        state = {
            ...
            "output_dir": "C:/tmp/paperpilot/tasks/task-local-pdf",
            "options": {
                "pdf_dir": "D:/papers",
