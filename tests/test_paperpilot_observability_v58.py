...
class PaperPilotObservabilityV58Test(unittest.TestCase):
    def test_recursive_redaction_masks_credentials_and_user_identity(self):
        from knowledge_storm.paperpilot_observability import sanitize_payload
    ...
    def test_local_exporter_records_trace_span_score_and_completion(self):
        from knowledge_storm.paperpilot_observability import PaperPilotObservability

        with tempfile.TemporaryDirectory() as root:
            observability = PaperPilotObservability(Path(root), enabled=False)
    ...
    def test_langfuse_exporter_is_optional_and_fail_open(self):
        ...
        with tempfile.TemporaryDirectory() as root:
            ...
            with observability.trace("paperpilot.chat", input={"message": "hello"}) as trace:
    ...
    def test_local_mirror_failure_is_fail_open_and_visible_in_status(self):
        ...
        with tempfile.TemporaryDirectory() as root:
            observability = PaperPilotObservability(Path(root), enabled=False)
            with mock.patch(
                "knowledge_storm.paperpilot_observability.Path.open",
                side_effect=OSError("disk unavailable"),
            ):
                with observability.trace("paperpilot.chat") as trace:
    ...
    def test_langfuse_exporter_builds_nested_observations_and_scores(self):
        ...
        root = client.roots[0]
        self.assertEqual(root.name, "paperpilot.chat")
    ...
    def test_nested_span_and_generation_preserve_parent_and_usage(self):
        ...
        with tempfile.TemporaryDirectory() as root:
            ...
            with observability.trace("paperpilot.research") as trace:
    ...
    def test_litellm_model_exports_each_call_as_generation(self):
        ...
        client = _FakeLangfuseClient()
        with tempfile.TemporaryDirectory() as root:
            observer = PaperPilotObservability(Path(root), enabled=True, langfuse_client=client)
            with observer.trace("paperpilot.research") as trace:
    ...
    def test_trace_recorder_bridges_local_stages_to_nested_langfuse_spans(self):
        from knowledge_storm.paperpilot_observability import PaperPilotObservability
        from knowledge_storm.paperpilot_trace import PaperPilotTraceRecorder
        ...
        with tempfile.TemporaryDirectory() as root:
            ...
            with observability.trace("paperpilot.research") as trace:
                with trace.span("research_pipeline") as pipeline:
                    recorder = PaperPilotTraceRecorder(
    ...
    def test_chat_spans_export_router_memory_and_generation_details(self):
        ...
        with tempfile.TemporaryDirectory() as root:
            observer = PaperPilotObservability(Path(root), enabled=True, langfuse_client=client)
            service = PaperPilotTaskService(Path(root), observability=observer)
            session = service.create_chat_session(run_mode="fake", user_id="alice")
            service.send_chat_message(session["chat_id"], "请记住：回答先给结论")

        root_trace = next(item for item in client.roots if item.name == "paperpilot.chat")
    ...
    def test_factory_is_disabled_without_credentials(self):
        from knowledge_storm.paperpilot_observability import build_observability
    ...
    def test_offline_test_mode_hard_disables_remote_export(self):
        ...
        environment = {
            "PAPERPILOT_TEST_OFFLINE": "1",
            "PAPERPILOT_OBSERVABILITY": "langfuse",
    ...
    def test_task_service_emits_research_and_chat_harness_traces(self):
        ...
        with tempfile.TemporaryDirectory() as root:
            observability = PaperPilotObservability(Path(root), enabled=False)
            service = PaperPilotTaskService(Path(root), observability=observability)
            ...
            self.assertIn("paperpilot.research", trace_names)
            self.assertIn("paperpilot.chat", trace_names)
    ...
    def test_benchmark_completion_exports_numeric_metrics_as_scores(self):
        from knowledge_storm.paperpilot_benchmarks import BenchmarkRegistry, BenchmarkRunManager
        from knowledge_storm.paperpilot_observability import PaperPilotObservability
    ...
    def test_v58_ui_and_readme_expose_langfuse_configuration(self):
        ...
        html = (root / "frontend" / "paperpilot_dashboard" / "index.html").read_text(
            encoding="utf-8"
        )
        script = (root / "frontend" / "paperpilot_dashboard" / "app.js").read_text(
            encoding="utf-8"
        )
        ...
        self.assertIn("PAPERPILOT_OBSERVABILITY", readme)
