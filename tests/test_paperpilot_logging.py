...
class PaperPilotLoggingTest(unittest.TestCase):
    ...
    def test_paperpilot_logging_filters_noisy_provider_warnings(self):
        from examples.storm_examples.run_paper_storm_minimax import (
            configure_paperpilot_logging,
        )

        configure_paperpilot_logging(verbose=False)
    ...
    def test_paperpilot_logging_filters_litellm_provider_stdout(self):
        from examples.storm_examples.run_paper_storm_minimax import (
            PaperPilotStdoutFilter,
            configure_paperpilot_logging,
        )
        ...
        try:
            configure_paperpilot_logging(verbose=False)
            self.assertIsInstance(sys.stdout, PaperPilotStdoutFilter)
    ...
    def test_paperpilot_logging_filters_legacy_wikipedia_errors(self):
        from examples.storm_examples.run_paper_storm_minimax import (
            configure_paperpilot_logging,
        )
    ...
    def test_trace_recorder_writes_jsonl_events_and_summary(self):
        ...
        with TemporaryDirectory() as tmpdir:
            trace = PaperPilotTraceRecorder(tmpdir)
    ...
    def test_traced_retrieval_model_records_success_and_error(self):
        ...
        with TemporaryDirectory() as tmpdir:
            recorder = PaperPilotTraceRecorder(tmpdir)
            rm = DummyRM()
