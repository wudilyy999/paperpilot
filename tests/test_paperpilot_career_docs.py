...
class PaperPilotCareerDocsTest(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.resume = (cls.root / "docs" / "PAPERPILOT_RESUME_GUIDE.md").read_text(
            encoding="utf-8"
        )
        ...
        cls.domain_report = (
            cls.root / "docs" / "PAPERPILOT_DOMAIN_PILOT.md"
        ).read_text(encoding="utf-8")
    ...
    def test_interview_playbook_has_one_hundred_questions_in_fixed_modules(self):
        ...
        module_matches = list(
            re.finditer(
                r"^## (基础原理|Bad Case 与排查|假设性系统设计|PaperPilot 针对性追问)$",
                self.playbook,
                flags=re.MULTILINE,
            )
        )
