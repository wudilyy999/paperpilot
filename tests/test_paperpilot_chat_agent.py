...
class PaperPilotChatAgentTest(unittest.TestCase):
    ...
    def test_legacy_messages_are_backfilled_with_visible_usage_metadata(self):
        from knowledge_storm.paperpilot_chat_agent import PaperPilotChatAgent

        service = self.make_service()
        agent = PaperPilotChatAgent(service)
    ...
    def test_chat_auto_researches_when_existing_task_has_insufficient_evidence(self):
        ...
        with mock.patch.dict(os.environ, {"PAPERPILOT_CHAT_LLM": "0"}), mock.patch.object(
    ...
    def test_casual_interview_question_gets_topic_aware_reply(self):
        ...
        self.assertNotEqual(
            content,
            "你好，我是 PaperPilot Research Agent。你可以聊天、查询长期记忆、问已有知识库，或启动论文调研与深度研究。",
        )
