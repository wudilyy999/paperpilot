...
class PaperPilotStageTraceTest(unittest.TestCase):
    def make_recorder(self):
        temp_dir = tempfile.TemporaryDirectory()
        self.addCleanup(temp_dir.cleanup)
        return PaperPilotTraceRecorder(temp_dir.name)
    ...
    def test_storm_callback_reports_persona_dialogue_and_evidence(self):
        recorder = self.make_recorder()
        callback = PaperPilotStageCallback(recorder, topic="PIM 神经网络抑制")
    ...
    def test_pipeline_attributes_bootstrap_connection_failure_to_request(self):
        from knowledge_storm.paperpilot_pipeline import (
            PaperPilotPipelineConfig,
            run_paperpilot_pipeline,
        )

        with tempfile.TemporaryDirectory() as temp_dir:
            config = PaperPilotPipelineConfig(
                topic="PIM",
                topic_for_storm="PIM",
            ...
            with mock.patch(
                "knowledge_storm.paperpilot_pipeline._build_lm_configs",
                side_effect=RuntimeError("OpenAIException - Connection error"),
            ):
                with self.assertRaises(RuntimeError):
                    run_paperpilot_pipeline(config)

            events = [
                json.loads(line)
                for line in (Path(config.article_dir) / "paperpilot_trace.jsonl")
    ...
    def test_pipeline_emits_real_storm_callback_and_module_stages(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with mock.patch(
                ...
                "knowledge_storm.paperpilot_pipeline._build_paper_retriever",
                return_value=lambda *args, **kwargs: [],
            ), mock.patch(
                "knowledge_storm.paperpilot_pipeline.STORMWikiRunner",
                FakeRunner,
            ), mock.patch(
                "knowledge_storm.paperpilot_pipeline.ensure_research_sources",
                return_value=1,
            ), mock.patch(
                "knowledge_storm.paperpilot_pipeline._write_pipeline_scorecard",
                side_effect=write_scorecard,
            ):
                result = run_paperpilot_pipeline(config)
