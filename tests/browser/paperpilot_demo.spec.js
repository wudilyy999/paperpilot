...
const screenshotRoot = process.env.PAPERPILOT_SCREENSHOT_DIR || "test-results/paperpilot-demo";
...
test("desktop one-click demo completes and renders the research result", async ({page}) => {
  ...
  await expect(page.locator(".release-pill strong")).toHaveText("v7.2");
...
test("developer workbench discovers local datasets and exposes reproducible runs", async ({page}) => {
  ...
  await expect(page.locator("#ready-root-detail")).toContainText("paperpilot-benchmarks");
